...
equations <- function(x) {
  ...
  eq3 <- Jh - ((1 - gamma) * (z_h - b) - gamma * cc * th_h + beta * (1 - s) * (pi_z * Jh + (1 - pi_z) * Jl))
  eq4 <- Jl - ((1 - gamma) * (z_l - b) - gamma * cc * th_l + beta * (1 - s) * (pi_z * Jl + (1 - pi_z) * Jh))
  c(eq1, eq2, eq3, eq4)
}
...
figname <- paste0(paste0("Q3/Figures/plot_q3_part3_sim_b_", as.character(b)), ".pdf")
...
dev.off()
cat("\nPlot saved to Q3/Figures/plot_q3_part3_sim.pdf\n")
