...
equations <- function(x) {
  ...
  eq3 <- Jh - ((z_h - w_bar) + beta * (1 - s) * (pi_z * Jh + (1 - pi_z) * Jl))
  eq4 <- Jl - ((z_l - w_bar) + beta * (1 - s) * (pi_z * Jl + (1 - pi_z) * Jh))
  c(eq1, eq2, eq3, eq4)
}
...
figname <- "Q3/Figures/plot_q3_part6_sim.pdf"
...
dev.off()
cat("\nPlot saved to Q3/Figures/plot_q3_part6_sim.pdf\n")
